...
简单的可以理解为 _`容器(Container)`就是系统中进行了特殊处理的进程_
...
`Docker` 默认镜像源在国外，镜像拉取会很慢，所以需要将镜像源改为国内

修改 `Docker` 默认源是修改 **/etc/docker/daemon.json（如果没有此文件，则自行创建）**，

然后在此文件中添加国内镜像，我在此添加了 _网易镜像源_ 、_中国科技大学镜像源_ 、_Docker 官方中国区镜像源_
...
重启完毕后可以使用 **docker info** 命令执行查看

<img src=./images/01/06_02.png width="50%" />

可以看到 **Registry Mirrors** 属性便是设置的国内镜像源。此时拉取镜像的速度便会快很多
